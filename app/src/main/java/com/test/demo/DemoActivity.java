...
import com.net.DisplayUtil;
import com.recycler.RecyclerCommonAdapter;
 ...
 * @modificationHistory
 */
public class DemoActivity extends AppCompatActivity implements Callback {
    ...
    public List<ResultBean.DataBean> mList = new ArrayList<>();
    ...
    private RecyclerCommonAdapter<ResultBean.DataBean> mAdapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mBottomRV=findViewById(R.id.m_bottom_rv);
        ...
        mTimer = new CountDownTimer(5 * 1000, 1000) {
            ...
            public void onFinish() {
                mManyLL.setVisibility(View.GONE);
                mList.clear();
                mIdList.clear();
                File file = new File("/mnt/sdcard/sdklog");
                if (file.exists()) {
                    file.delete();
                }

                if(getFileOrFilesSize(LogcatHelper.PATH_LOGCAT,3)>2048){
                    LogcatHelper.getInstance(getApplicationContext()).stop();
                    deleteFolderFile(LogcatHelper.PATH_LOGCAT,true);
...
//        m_oIPAddr.setText("10.18.39.13");
        m_oIPAddr.setText("192.168.1.65");
        m_oPort.setText("8000");
        m_oUser.setText("admin");
        m_oPsd.setText("juhe1234");
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        m_osurfaceView.getHolder().setFormat(PixelFormat.RGBA_8888);
...
//            params.bottomMargin = 0;
    ...
    private void findViews() {
        ...
        mUpRl = (RelativeLayout) findViewById(R.id.widget_Play);
        mManyLL = findViewById(R.id.m_many_ll);
        mRV = findViewById(R.id.m_rv);
...
//            JNATest.TEST_Config(m_iPlayID, m_iLogID, m_iStartChan);
            for (int i = 0; i < 20; i++) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        JNATest.jnaTest.Test_FindPicture(m_iLogID, m_iStartChan);
                    }
                }).start();
            }
        ...
        public void onClick(View v) {
            try {
                if (m_iLogID < 0) {
                    ...
                    JNATest.jnaTest.Test_Alarm_V41(m_iLogID, DemoActivity.this);
    ...
    private Button.OnClickListener Preview_Listener = new Button.OnClickListener() {
        public void onClick(View v) {
            try {
                ...
                mUpRl.setVisibility(View.GONE);
                m_osurfaceView.setZOrderOnTop(false);
                m_osurfaceView.setZOrderMediaOverlay(false);
            } catch (Exception ex) {
...
//            String sendParam = "accessToken=" + accessToken + "&domain=" + areaDomain;  
    ...
    public void showRecyclerView() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                mTimer.start();
                mManyLL.setVisibility(View.VISIBLE);

                if (mAdapter == null) {
                    mAdapter = new RecyclerCommonAdapter<ResultBean.DataBean>(
                            DemoActivity.this,
                            R.layout.item_people, mList) {
                        @Override
                        protected void convert(ViewHolder holder, ResultBean.DataBean dataBean, int position) {
                            holder.setText(R.id.m_name_tv, dataBean.getName());
                            RoundedImageView topIV = holder.getView(R.id.m_top_iv);
                            Glide.with(DemoActivity.this)
                                    .load(RequestManager.mBaseUrl + "analyze/download/" +dataBean.getIcon()).into(topIV);
                            Log.e(TAG,RequestManager.mBaseUrl + "analyze/download/" +dataBean.getIcon());
                            TextView noTV = holder.getView(R.id.m_no_tv);
                            noTV.setText(dataBean.getEmployeeNo());
                            if (dataBean.getEmployeeNo() == null || TextUtils.isEmpty(dataBean.getEmployeeNo())) {
                                noTV.setVisibility(View.GONE);
                            } else {
                                noTV.setVisibility(View.VISIBLE);
                            }
                            holder.setText(R.id.m_no_tv, dataBean.getEmployeeNo());
                            holder.setText(R.id.m_position_tv, (dataBean.getDepartment() == null ? "" : dataBean.getDepartment()) + "\n" + (dataBean.getTitle() == null ? "" : dataBean.getTitle()));
                            LinearLayout mRootLL = holder.getView(R.id.m_root_ll);
                            RecyclerView.LayoutParams layoutParams = (RecyclerView.LayoutParams) mRootLL.getLayoutParams();
                            layoutParams.width = DisplayUtil.getScreenWidth(mContext) / 6;
                        }
                    };
                ...
                if(mList.size()>6){
                    mRV.smoothScrollToPosition(mList.size()-1);
                }
            }
        });
